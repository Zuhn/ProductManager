...
NSString * MademoiselleCharlotteAdmin$ModuleRequireFormat = @"(function(exports){"
...
@implementation MademoiselleCharlotteAdminObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_mademoisellecharlotteadmin);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_mademoisellecharlotteadmin gc];
}
...
{
	// create MademoiselleCharlotteAdmin global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "MademoiselleCharlotteAdmin" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_mademoisellecharlotteadmin = [[MademoiselleCharlotteAdminObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_mademoisellecharlotteadmin];
	
	NSString *_mademoisellecharlotteadminNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _mademoisellecharlotteadminNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_mademoisellecharlotteadmin valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:MademoiselleCharlotteAdmin$ModuleRequireFormat,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.mademoisellecharlotteadmin.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.mademoisellecharlotteadmin.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
