...
//Rare exceptions to indicate a bug in the _mademoisellecharlotteadmin code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_mademoisellecharlotteadmin = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_mademoisellecharlotteadmin]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.mademoisellecharlotteadmin.module"
